...
export default class CommunityManager extends BaseManager<Community> {
	...
	async fetchCommunity({
		...
		let promiseResolve!: (
			value: Community | PromiseLike<Community | null> | null
		) => void
	...
	async setGuildConfig({
		...
		const req = await fetch(
			`${this.apiurl}/discord/guilds/${config.guildId}`,
			{
				method: "PATCH",
				body: JSON.stringify(config),
				credentials: "include",
	...
	async revokeApikeys({
		...
		const result = await this.manageApikey({
			create: true,
			invalidate: true,
			reqConfig,
		})
	...
	async masterCreateKey({
		...
		const result = await this.masterManageApikey({
			communityId,
			create: true,
			keyType,
			reqConfig,
	...
	async notifyGuildConfigChanged({
		...
		const req = await fetch(
			`${this.apiurl}/discord/guilds/${strictUriEncode(
				guildId
			)}/notifyChanged`,
			{
				method: "POST",
	...
	async merge({
		...
		const req = await fetch(
			`${this.apiurl}/communities/${strictUriEncode(
				idReceiving
		...
		if (merge?.error)
			throw new GenericAPIError(`${merge.error}: ${merge.message}`)
