...
const AddRule: Command = {
	name: "addrule",
	description: "Adds a rule filter. Please see the [Explanation](https://gist.github.com/oof2win2/370050d3aa1f37947a374287a5e011c4#file-trusted-md)",
	aliases: [ "addrules" ],
	usage: "[...ids]",
	examples: [
		"addrule XuciBx7",
		"addrule XuciBx7 XuciBx9 XuciBx/",
	],
	category: "rules",
	requiresRoles: true,
	...
	run: async ({client, message, guildConfig, args}) => {
		...
		if (!args[0]) {
			...
			for (let i = 0; i < allRules.length; i += 2) {
				fields.push({
					...
					name: allRules[i + 1] && `${allRules[i + 1].shortdesc} (\`${allRules[i + 1].id}\`)`,
					inline: false
				})
			}
		...
		if (!newRules.length) return message.channel.send("No valid or non-duplicate rules to be added")
		...
		const newRuleIds = new Set([...guildConfig.ruleFilters, ...newRules.map((r) => r.id)])
