...
const AllReports: Command = {
	...
	run: async ({client, message, args}) => {
		...
		if (!reports[0]) return message.channel.send(`Player \`${playername}\` doesn't have any reports`)
		...
		const fields = await Promise.all(
			reports.map(async (report) => {
				...
				const rule = await client.fagc.rules.fetchRule({ ruleid: report.brokenRule })
				const community = await client.fagc.communities.fetchCommunity({ communityId: report.communityId })
				return {
					name: report.id,
