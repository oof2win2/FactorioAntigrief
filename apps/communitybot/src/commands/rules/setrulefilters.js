...
class SetRuleFilters extends Command {
	constructor(client) {
		super(client, {
			name: "setrulefilters",
			description:
				"Sets rule filters. [Explanation](https://gist.github.com/oof2win2/370050d3aa1f37947a374287a5e011c4#file-trusted-md)",
			aliases: [],
			category: "rules",
			dirname: __dirname,
			enabled: true,
	...
	async run(message, _, config) {
		...
		let embed = new MessageEmbed()
			...
			.setDescription(
				"Set Filtered Rules. [Explanation](https://gist.github.com/oof2win2/370050d3aa1f37947a374287a5e011c4#file-trusted-md)"
			)
		...
		message.channel.send(
			"Please type in IDs of rules you wish to use. Type `stop` to stop"
		)
		...
		const onEnd = async () => {
			let embed = new MessageEmbed()
				...
				.setAuthor("FAGC Community")
				.setDescription(
					"Filtered Rules. [Explanation](https://gist.github.com/oof2win2/370050d3aa1f37947a374287a5e011c4#file-trusted-md)"
				)
			const fields = ruleFilters
				.map((filteredRuleID) => {
			...
			try {
				const confirm = await getConfirmationMessage(
					message,
					"Are you sure you want your rule filters set to this?"
				)
				if (!confirm)
				...
				if (error instanceof AuthenticationError)
					return message.channel.send(
						"Your API key is set incorrectly"
					)
				message.channel.send(
		...
		let collector = await message.channel.createMessageCollector(
			messageFilter,
			{ max: Object.keys(rules).length, time: 120000 }
		)
