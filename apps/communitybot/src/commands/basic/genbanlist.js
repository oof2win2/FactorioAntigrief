...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        let trustedCommunitiesProm = config.trustedCommunities.map(id => {
            return fetch(`${client.config.apiurl}/communities/getid?id=${id}`).then(res => res.json())
        })
        ...
        let rulePromises = config.ruleFilters.map((rule) => {
            return fetch(`${client.config.apiurl}/violations/getbyrule?id=${rule}`).then(res => res.json())
        })
        ...
        let banlist = violationArr.map((violation) => {
            return {
                ...
                reason: `Banned on FAGC. Please check one of the community Discord servers or go to ${client.config.apiurl}/offenses/getall?playername=${violation.playername}`
            }
        })
